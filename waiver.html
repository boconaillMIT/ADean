...
<body>
  ...
  <script>
    ...
    var OTPI_LABELS = {
      ...
      emeritus_confirmation:"Emeritus / post-tenure status confirmation"
    };
    ...
    var OTPI_TYPE_LABEL = {
      ...
      emeritus: "Emeritus / post-tenure / post-retirement"
    };
    ...
    function draftOtpiPlata() {
      var o = (lastResult && lastResult.one_time_pi) || {};
      var f = o.facts || {};
      var type = otpiType(o);
      var html = (type === "standard") ? plataStandard(f) : plataNonStandard(type, f);
      var subj = "One-time PI status recommendation" + (f.proposed_pi ? " - " + f.proposed_pi : "");
      showNewMessagePreview(html, PLATA_EMAIL, subj);
    }

    // Standard (developing researcher): lead-PI framing + B6 prior-history line.
    function plataStandard(f) {
      ...
      return "Hi Professor Plata,<br><br>" +
    ...
    // Post-tenure / emeritus (and incoming faculty): no development framing, no B6.
    function plataNonStandard(type, f) {
      var pi = slot(f.proposed_pi, "professor");
      var statusClause = (type === "incoming_faculty")
        ? "an incoming faculty member"
        : "who holds " + slot(f.status_label, "post-tenure/emeritus") + " status";
      return "Hi Professor Plata,<br><br>" +
        slot(f.dlc,"DLC") + " is requesting one-time PI status for " + pi + ", " + statusClause +
        ", for a " + slot(f.sponsor,"sponsor") + " proposal (" + slot(f.proposal_title,"proposal title") +
        "). The request has department approval.<br><br>" +
        slot(f.reason_summary, "one line on the professor's role / expertise on the project") + "<br><br>" +
        "The proposal period is " + slot(f.proposal_period,"period") + " and if awarded, the funding would provide " +
        slot(f.salary_support,"salary support") + " of salary support each year.<br><br>" +
        "I reviewed the submission and recommend it for approval. Please let me know if you agree and/or if you have any further questions.<br><br>Sincerely,";
    }
